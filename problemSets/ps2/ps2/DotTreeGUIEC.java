...
public class DotTreeGUIEC extends DrawingGUI {
    ...
    private int recHeight = 20;                     // For the rectangle around the mouse's height
    private int recWidth = 40;                      // For the rectangle around the mouse's width
    private boolean trackMouse = false;				// if true, then print out where the mouse is as it moves
    private ArrayList<Dot> found = null;					// who was found near mouse, when querying

    public DotTreeGUIEC() {
        super("dottreeEC", width, height);
    }
    ...
    public void handleMousePress(int x, int y) {
        if (mode == 'a') {
            // If there aren't any points, make a new tree and add give it a root
            if (this.tree == null) {
                tree = new PointQuadtree<>(new Dot(x, y), 0, 0, width, height);
            } else {
                // Inset point to tree as mouseX and mouseY
            ...
            found = tree.findInRectangle(x,y,recWidth,recHeight);
        }
        else {
    ...
    /**
     * test tree 2 -- Made by Ben and Alex
     * hardcoded point locations for 800x600
     */
    public void test2() {
        // Diagonal test
        found = null;
        tree = new PointQuadtree<Dot>(new Dot(75,450), 0,0,800,600); // start with A
        tree.insert(new Dot(150,375)); // B
        tree.insert(new Dot(225,300)); // C
        tree.insert(new Dot(300,225)); // D
        tree.insert(new Dot(375, 150)); // E
        tree.insert(new Dot(450,75)); // F

        int bad = 0;
        bad += testFind(74,451,10,3,2,1);	// rect for A;  hit A
        bad += testFind(50,451,10,2,1,0);	// rect for A; find none
        bad += testFind(320,245,35,6,5,1);	// rect for all; Circle for A,B,C,D,E; find D
        bad += testFind(width/2,height/2,400,6,6,6);	// rect for all; circle for all;find all
    ...
    public void handleKeyPress(char key) {
        if (key=='a' || key=='q' || key == 'r') {
            mode = key;
            found = null;
        }
        else if (key=='+' || key=='=') {
            // Keeps track of both modes
            mouseRadius += 10;
            recHeight += 5;
            recWidth += 10;
        }
            ...
            recHeight -= 5;
            if (recHeight <0) recHeight = 0;
            recWidth -= 10;
            if (recWidth < 0) recWidth = 0;
        }
        else if (key=='m') {
    ...
    public void draw(Graphics g) {
        ...
        if (mode == 'r'){
            ...
            g.drawRect(mouseX-(recWidth/2),mouseY-(recHeight/2),recWidth,recHeight);
            if (found != null) {
                g.setColor(Color.black);
                for (Dot d : found) {
                    g.fillOval((int)d.getX()-dotRadius, (int)d.getY()-dotRadius, 2*dotRadius, 2*dotRadius);
